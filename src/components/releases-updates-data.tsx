...
import { LucideArrowUpCircle, LucideRefreshCw } from 'lucide-react';
...
export function RefreshReleasesButton() {
  ...
  const { isRefetching, refetch } = useQuery(queryOpts({ id: id as string }));
...
export function ReleasesUpdatesData() {
  const router = useRouter();
  const { id } = router.query;
          ...
          {releases
            ...
            .map((release, index) => (
              <TableRow key={release.timestamp}>
                ...
                <TableCell>
                  {index === 0 ? (
                    <Badge className="h-8 rounded-md px-3">
                      <span>
                        <LucideArrowUpCircle className="size-4" />
                      </span>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <div className="space-y-2">
                          <Badge variant="outline" className="w-full justify-center p-4 truncate">
                            {release.commitHash.slice(0, 7)} - {release.commitMessage}
                          </Badge>
                        </div>
                        <AlertDialogFooter>
